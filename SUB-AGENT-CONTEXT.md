...
1. **Options:** Present 2-3 options with recommended choice + reasoning. Include "do nothing" when reasonable.
2. **Confidence (MANDATORY):** Tag EVERY non-trivial response with `[X% confident — because Y, uncertain about Z]`
...
3. **Research first:** Never guess. If unsure, search the web or check files before recommending.
4. **System thinking:** If you fix a bug, create a rule that prevents the entire CLASS of bugs. Add to this file.
5. **Compound:** Every task should leave behind knowledge (file, rule, checklist) that makes the next task easier.
6. **Self-audit before completing:** Re-read original requirements. What's missing? Rate confidence. Flag uncertainties.
