...
## Paper 32: AgentBench - Evaluating [[LLM]]s as Agents
...
Die Evaluation von 29 [[LLM]]s (API-based: GPT-4, Claude, Gemini; Open-Source: Llama-2, Vicuna, CodeLlama) offenbart drastische Performance-Disparitäten. GPT-4 erreicht eine Overall AgentBench Score von 4.01, während selbst die stärksten Open-Source Modelle (CodeLlama-34B: 0.96, Llama-2-70B: 0.78) dramatisch zurückbleiben - ein Gap von ~4-5x. Die Autoren normalisieren Scores über Tasks (Weight-1 = Durchschnitt aller getesteten [[LLM]]s pro Task), um Task-Schwierigkeitsunterschiede zu kompensieren.
...
"Your [[AI]] Agent scored 95% on MMLU. Congratulations - that's completely irrelevant. AgentBench just proved that Open-Source [[LLM]]s are 4-5x worse than GPT-4 at the ONE thing that actually matters: being an agent. Here's why every [[AI]] benchmark you're using is lying to you."
...
"Open-Source [[LLM]]s are not agent-ready. You can fine-tune Llama-2-70B all you want - you're not closing a 4x performance gap without fundamentally better alignment. Use GPT-4 for production agents, or wait for the next generation of Open-Source models trained explicitly for agent tasks."

**Call-to-Action:**
"If you're building an [[AI]] agent for a real product (not a demo), download AgentBench. Test your candidate [[LLM]]s. Measure the gap. Because the gap between 'works in demo' and 'works for customers' is quantified now - and it's bigger than you think."
...
- **Screenshot:** Für Vision-[[LLM]]s (erkenne Maschinen-Diagrams, Quality-Photos)
...
- **Agent-Computer Interface (ACI) Concept:** Einführung eines neuen Paradigmas - [[LLM]]s sind unique End-Users die custom Interfaces brauchen, analog zu IDEs für Human-Developers (statt nur "throw [[LLM]] at problem")
...
6. **Keine User Study:** Claims über "[[LLM]]s sind neue End-Users" basieren auf Designer-Intuition, nicht systematischer Study welche Interface-Elements [[LLM]]s tatsächlich nutzen/ignorieren
...
class ResearchAgentInterface:
    ...
    def extract_key_findings(self, paper_url):
        ...
        findings = self.[[LLM]]_analyzer.extract_findings(content)
        return {
...
- Option B: Custom interface designed for how [[LLM]]s think
...
- **[[LLM]]-Based Zero-Shot Coordination:** Erste erfolgreiche Anwendung von [[LLM]]s für proactive Multi-Agent Coordination ohne Training auf spezifischen Teammates - ersetzt Population-Based Training durch reasoning-based adaptation
...
Agent-FLAN adressiert die fundamentale Frage: Wie integriert man agent abilities in general-purpose [[LLM]]s ohne deren general capabilities zu verschlechtern? Das Paper identifiziert drei kritische Observations die bisherige Agent-Tuning Ansätze limitieren: (1) **Format-Reasoning Entanglement** - Current agent training data vermischt format following (ReAct-Structure, JSON) mit actual reasoning, was zu inadequate learning führt da Models overfitting auf format statt reasoning, (2) **Heterogeneous Learning Speeds** - [[LLM]]s lernen verschiedene Agent-Capabilities (retrieval, reasoning, understanding, instruction-following) mit unterschiedlichen Geschwindigkeiten, was uniform training inefficient macht, (3) **Hallucination Neglect** - Bisherige Approaches fokussieren auf positive cases, ignorieren aber hallucination issues die in production kritisch sind.
...
Here's something nobody talks about: [[LLM]]s learn different agent-skills at DIFFERENT speeds.
...
```python
from transformers import AutoModelForCausa[[LLM]], Trainer

# Load base model
model = AutoModelForCausa[[LLM]].from_pretrained("meta-llama/Llama-2-7b")
...
"Open-source [[LLM]]s have a dirty secret: They're all one-trick ponies. Llama-2 is great at text, terrible at code. CodeLlama is great at code, mediocre at text. For [[AI]] agents, this is fatal. Lemur just fixed it - and it's the first open-source model that's actually agent-ready."
