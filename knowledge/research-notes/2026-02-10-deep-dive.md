...
Diese drei Papers definieren die **wissenschaftliche Grundlage für moderne Multi-Agent [[AI]] Systeme**. Sie liefern:
1. **Ein Framework** für agentic reasoning (Paper 1)
...
**Direkte Anwendbarkeit für OpenClaw:** ✅ Hoch — alle drei Papers liefern konkrete Patterns, die wir HEUTE einbauen können.
...
### 1. Full Summary (500 Wörter)

Dieses Survey Paper systematisiert **Agentic Reasoning** — die Fähigkeit von [[LLM]]s, nicht nur zu *denken*, sondern zu *handeln*. Die zentrale These: [[LLM]]s entwickeln sich von passiven Textgeneratoren zu autonomen Agenten, die **planen, handeln und lernen** durch kontinuierliche Interaktion.

**Das 3-Layer Framework:**

**Layer 1: Foundational Agentic Reasoning** etabliert die Grundfähigkeiten:
...
- **Search:** Exploration von Lösungsräumen (z.B. MCTS-basierte Ansätze)
...
- **Feedback Mechanisms:** Reflexion über eigene Outputs (z.B. Reflexion Framework)
- **Memory:** Persistente State-Updates über Tasks hinweg (episodic + semantic memory)
- **Adaptation:** Policy-Updates durch Experience-Accumulation (z.B. RL-for-memory)

Der entscheidende Shift: Agents entwickeln **Memories und Policies**, die *über einzelne Tasks hinweg* wirken — echtes kontinuierliches Lernen.
...
Problem: **Complexity führt zu instability** — ohne strukturierte Koordination degeneriert Communication in "noise".

**Optimization Modes durchziehen alle Layer:**
- **In-Context:** Test-time orchestration durch Prompts/Workflows (z.B. ART, GEAR)
- **Post-Training:** Parameter-Updates via RL (z.B. GRPO, ARPO für tool-use)

**Formale Basis:** Das Paper formalisiert Agents als POMDP mit Reasoning-Variable Z:
```
π(z_t, a_t | h_t) = π_reason(z_t | h_t) · π_exec(a_t | h_t, z_t)
```
→ Explizite Trennung: **Think** (internal reasoning in Z) vs. **Act** (external action in A)
...
- ✅ **Unified Framework:** Erste systematische Taxonomie über Foundational → Self-Evolving → Collective Intelligence
- ✅ **Formale Definition:** Agentic Reasoning als POMDP mit expliziter Think/Act-Dekomposition
- ✅ **Optimization Taxonomy:** Klare Trennung In-Context (Prompt-Engineering) vs. Post-Training (RL/SFT)
- ✅ **Comprehensive Survey:** 160+ Seiten mit state-of-the-art Methods, Applications, Benchmarks
- ✅ **Practical Patterns:** Konkrete Workflows (ReAct, Tree-Search, Feedback-Loops) mit Code-Repos
- ✅ **Meta-Learning Loop:** Formalisierung von Self-Evolution als S_{k+1} ← U(S_k, τ_k, F_k)
- ✅ **Multi-Agent Formalization:** Dec-POMDP mit Communication Channel als Teil der Observation

### 3. Kritik / Limitationen

❌ **Zu breit, zu wenig tief:** 160 Seiten decken *alles* ab → wenig Depth in einzelnen Bereichen  
❌ **Fehlende Empirics:** Kein systematischer Benchmark-Vergleich der vorgestellten Patterns  
❌ **Cost-Ignoranz:** Kein Framework für Trade-offs (accuracy vs. token-cost vs. latency)  
❌ **Orchestration Overhead:** In-Context-Patterns (ReAct, MCTS) haben massive Token-Costs — wird nicht diskutiert  
❌ **Multi-Agent Stability:** Collective Reasoning hat bekannte Failure Modes (Paper erwähnt es, löst es nicht)  
❌ **Skill Selection:** Erwähnt Tool-Use, aber nicht das Scaling-Problem (wird erst in Paper 3 gelöst)  

### 4. Direkte Anwendbarkeit für uns (konkret)
...
**Benefit:** Bessere Interpretability + kann Reasoning-Traces für Post-Training sammeln

**C. Memory System nach Paper-Pattern:**
- **Flat Memory:** `MEMORY.md` (current) → ✅ haben wir
...
Paper zeigt: Post-Training via RL > SFT-only für adaptive Tool-Use.
```
Konkrete Idee:
- Log successful tool-use trajectories (task → reasoning → tool_call → result)
- Fine-tune on high-quality trajectories (SFT bootstrap)
- Optional: RL layer für outcome-based rewards
```
...
#### 🧪 **Experimentell (requires validation):**

**F. Tree-of-Thoughts for Complex Tasks:**
Paper zeigt MCTS-basierte Planning für hard reasoning tasks (HumanEval, Math).
```
Potential Use Case:
- Complex research tasks (RESEARCHER agent)
- Multiple solution paths → explore tree → verify best path
```
**Caveat:** Token-intensive — nur für high-value tasks

**G. Post-Training für King:**
Sammle trajectories von successful task completions → fine-tune base model.
```
Flow:
1. King completes task successfully
2. Log: initial_prompt → reasoning_steps → tool_calls → result
3. Accumulate dataset
4. SFT on successful patterns
```

**H. Multi-Agent Memory Sharing:**
Paper erwähnt: Shared Memory Systems für Collective Intelligence.
```
Idea:
...
### 5. Content-Angle: Worüber könnte Florian einen Artikel schreiben?

#### 🔥 **High-Impact Content Ideas:**

**1. "The 3 Layers of [[AI]] Agents: From Tools to Autonomous Minds"**
- Angle: Foundational → Self-Evolving → Collective als Evolution Path
...
- Angle: Self-Evolving Layer — Memory als Differentiator
...
- Angle: Optimization Modes — when to prompt vs. when to train
...
- Angle: Collective Layer — coordination overhead + failure modes
...
- Angle: Formalization — explicit reasoning traces
...
Dieses Paper ist ein **Manifest für wissenschaftliches Multi-Agent Design**. Die Kernthese: Das Feld steckt im "blind trial-and-error" fest, weil zwei fundamentale Dinge fehlen:
1. **Keine unified metric** um genuine collaboration gain von bloßem resource scaling zu trennen
...
→ **Ambiguity of Attribution** — niemand weiß, was wirklich funktioniert.

**Die Lösung: Collaboration Gain Γ**
...
```
Wo:
- Φ_M = Performance der Multi-Agent System
- Φ_S = Performance eines Single-Agent mit *gleichem* Resource-Budget

**Interpretation:**
- Γ = 1 → Null-Hypothese: Keine Kollaboration, nur resource scaling
- Γ > 1 → Genuine collaboration gain ("the whole > sum of parts")
- Γ < 1 → **Negative interference** — Multi-Agent macht es *schlimmer*

**Der Clou:** Φ_S ist nicht "dumb single agent", sondern **best possible single-agent baseline** unter same budget (z.B. best-of-N sampling, extended chain-of-thought).

**Factor Attribution Paradigm:**

Zweistufiger Prozess:
1. **Performance Check:** Führt Faktor-Änderung zu besserer Task-Performance?
2. **Γ-Evaluation:** Ist Γ > 1? (Wenn nein → "mere resource accumulation")

Nur wenn *beide* Tests bestehen → Faktor ist **genuine collaboration driver**.
...
- Sequential Dependency: Müssen Steps sequenziell sein?
...
- Content Entropy: H_t = -Σ p(x_i|C_t) log p(x_i|C_t) → Convergence-Tracking
- Evolutionary Distance: D_t = Σ (1 - cosine_similarity(v_i,t, v_i,t-1)) → Semantic displacement
...
- **Evolutionary Distance zu groß** = Contextual breakdown (schlecht)

→ Balance zwischen beiden = effective collaboration

**Empirical Guidance:**

Paper zeigt in Experimenten:
- **Agent Scale:** Γ > 1 bis ~50 agents, dann Koordinations-Overhead dominiert
...
- ✅ **Γ Metric:** Erste rigorous Metrik für genuine collaboration (decoupled from resources)
- ✅ **Factor Library:** Systematische Taxonomie des MAS Design Space
- ✅ **Attribution Paradigm:** Scientific method für Factor → Performance Causality
- ✅ **Information-Level Metrics:** Content Entropy + Evolutionary Distance als real-time diagnostics
- ✅ **Empirical Validation:** Experimente zeigen wo Multi-Agent wins/fails
- ✅ **Practical Framework:** Nicht nur Theorie — gibt konkrete Optimization-Schritte
- ✅ **Call to Action:** Fordert Community auf, Γ als Standard zu adoptieren

### 3. Kritik / Limitationen

❌ **Γ Computation ist non-trivial:** "Best single-agent baseline" erfordert careful tuning — kann manipuliert werden  
❌ **Resource Equivalence ist fuzzy:** Was heißt "same budget"? (Tokens? [[API]] calls? Latency?)  
❌ **Causality vs. Correlation:** Factor Attribution findet correlation, nicht necessarily causation  
❌ **Limited Empirics:** Viele Claims ohne extensive Experimente (mehr Manifesto als Paper)  
❌ **Information-Level Metrics:** Content Entropy / Evolutionary Distance sind interessant, aber unklar wie actionable  
❌ **Fehlende Failure Cases:** Wenig Detail über *warum* MAS mit Γ < 1 scheitert

### 4. Direkte Anwendbarkeit für uns (konkret)

#### 🔥 **Sofort anwendbar:**

**A. Implement Γ-Tracking für OpenClaw Sub-Agents:**
...
def measure_collaboration_gain(task, sub_agent_system, budget):
    ...
    Φ_M = evaluate_quality(result_mas)
    
    # Single-Agent Baseline (same budget)
    result_sas = king_agent.execute(task, budget, mode="best_of_n")
    Φ_S = evaluate_quality(result_sas)
    
    Γ = Φ_M / Φ_S
    
    if Γ > 1:
        log("✅ Genuine collaboration gain: {Γ:.2f}x")
    else:
        log("⚠️ Multi-agent underperforms single-agent baseline")
    
    return Γ
```

**Concrete Use Case:**
- HUNTER Sub-Agent für [[VC]] job search
...
```
Checklist:
1. Task Decomposability: ✅ VC research = parallelizable (companies, people, funds)
2. Sequential Dependency: ⚠️ Medium (need basic research before deep-dive)
3. Agent Diversity: ✅ HUNTER hat specialized prompts + tools (LinkedIn, Harmonic)
4. Expected Γ: > 1? → If unsure, run A/B test
```

**C. Communication Pruning für Sub-Agents:**
...
**D. Real-Time Γ Monitoring Dashboard:**

```
OpenClaw Dashboard:
- Per-task Γ tracking
- Alert wenn Γ < 1 for X consecutive tasks → Sub-Agent is net-negative
- Automatic suggestion: "Disable HUNTER, use King directly"
```

**E. Factor Attribution für neue Features:**
...
3. Measure Γ:
   - If Γ > 1 → Feature is genuine collaboration driver → KEEP
   - If Γ ≤ 1 → Feature is mere resource sink → DROP
```

#### 🧪 **Experimentell:**

**F. Information-Level Metrics für Debugging:**

Wenn Sub-Agent underperforms:
```python
# Track Content Entropy across Sub-Agent turns
H_t = -Σ p(output_i | context_t) log p(output_i | context_t)

If H_t stays high → Agent is "searching" (exploring)
If H_t drops fast → Agent converged (exploiting)
If H_t oscillates → Contextual breakdown (BUG)
```

**Use Case:** RESEARCHER agent doing multi-hop search — track wenn Entropy-Trajectory ist pathologisch.
...
def optimal_agent_count(task):
    for n in [1, 2, 3, 5, 10]:
        Γ_n = measure_gamma(task, n_agents=n)
    
    return argmax(Γ_n)  # Return scale with highest Γ
```
...
**1. "The Γ Metric: How to Measure if Your [[AI]] Agents Actually Collaborate"**
- Angle: Introduce Γ to practitioner audience
...
- Include: Decision tree (Task Decomposability → Agent Diversity → Communication)
...
- Include: Full framework (Γ + Factor Library + Attribution Paradigm)
...
Dieses Paper stellt die provokante Frage: **Wann sind Multi-Agent Systems eigentlich unnötig?** Die Antwort: Wenn ein Single-Agent mit einer **Skill Library** das gleiche erreichen kann — bei 50% weniger Latency und 54% weniger Tokens.

**Die Kernidee: MAS → SAS Compilation**

Multi-Agent System = Specialized Agents kommunizieren via natural language  
Single-Agent with Skills = Ein Agent wählt aus Skill Library

**Mathematisch:**
```
Multi-Agent: Route zwischen Agents (a_i → a_j)
Single-Agent: Wähle Skill aus Library (σ: context → skill)
```

**Compilation Process:**
```
For each Agent a_i with role ρ_i:
1. Extract capabilities: K_i = decompose(ρ_i)
2. Assign backend: ξ = {tool ∈ T | internalized}
...
- ✅ **Accuracy:** SAS ≈ MAS (±2%)
- ✅ **Tokens:** -54% average (MAS has context duplication)
- ✅ **Latency:** -50% average (eliminates inter-agent overhead)
- ✅ **[[API]] Calls:** 3-4 → 1 (direct cost savings)

**But dann kommt das Problem: Skill Selection Scaling**

Wenn Skill Library wächst: Wie gut kann [[LLM]] noch das *richtige* Skill auswählen?

**Key Finding: Phase Transition**

Accuracy degradiert nicht linear, sondern:
- **|S| < κ** (capacity threshold ~50-90): Accuracy bleibt > 90%
- **|S| > κ**: Sharp drop (phase transition)

**Fitted Scaling Law:**
```
Accuracy ≈ α / (1 + (|S| / κ)^γ)

GPT-4o-mini: κ = 91.8, γ = 1.72
GPT-4o:      κ = 83.5, γ = 1.56
```

**Critical Insight: Semantic Confusability > Library Size**

Experiment: Add "competitor skills" (ähnliche Beschreibungen, unterschiedliche Funktionen)
- **No competitors:** 100% accuracy bei |S|=20
- **1 competitor pro skill:** -30% accuracy
- **2 competitors:** -63% accuracy

→ **Shepard's Universal Law of Generalization:** Confusion ∝ e^(-semantic_distance)

**The Solution: Hierarchical Routing**

Flat selection breaks bei |S| > κ.  
...
- **Hick's Law:** RT = a + b·log₂(n) — bricht bei n>8 zusammen
- **Miller's 7±2:** Working memory capacity limits
- **ACT-R Fan Effect:** Mehr cues → weniger activation pro fact
- **Chunking Theory:** Experts manage 7 chunks, nicht 32 pieces

→ [[LLM]]s zeigen analoge capacity-limited behavior

### 2. Key Contributions

- ✅ **MAS → SAS Compilation:** Erste formale Transformation (inkl. Algorithm)
- ✅ **Efficiency Proof:** Empirisch zeigt SAS ~50% savings bei comparable accuracy
- ✅ **Scaling Law:** Fitted model ACC ≈ α / (1 + (|S|/κ)^γ) with R² > 0.97
- ✅ **Phase Transition Discovery:** Non-linear degradation, nicht gradual
- ✅ **Confusability > Size:** Semantic similarity ist primary failure mode
- ✅ **Hierarchical Mitigation:** +40% accuracy recovery through structured routing
- ✅ **Cognitive Grounding:** Verbindet [[LLM]] behavior mit human decision-making theory
- ✅ **Practical Guidelines:** Konkrete thresholds (κ ~50-100) + design recommendations

### 3. Kritik / Limitationen

❌ **Synthetic Skills Only:** Alle Experimente mit synthetic skill libraries (nicht real-world)  
❌ **Selection ≠ Execution:** Paper misst nur selection accuracy, nicht end-task performance  
❌ **Limited Models:** Nur GPT-4o + GPT-4o-mini getestet (generalizability unklar)  
❌ **Compilability Conditions zu restriktiv:** C1 (serializable), C2 (shared history), C3 (homogeneous) schließen viele real-world MAS aus  
❌ **Hierarchy Design underexplored:** Nur simple 2-level hierarchy — optimales Design offen  
❌ **No Latent Skills:** Fokus auf prompts, nicht auf learned/fine-tuned skills  

### 4. Direkte Anwendbarkeit für uns (konkret)

#### 🔥 **Sofort anwendbar:**

**A. Evaluate OpenClaw Sub-Agents für Compilation:**

**Compilability Check:**
```
Sub-Agent: HUNTER (VC job search)
- C1 Serializable? ✅ (King → HUNTER → King)
- C2 Shared History? ✅ (kein private state)
- C3 Homogeneous? ✅ (same base model)
→ COMPILABLE

Alternative: Skill Library für King
{
...
- Stateless tasks → Compile to skills
- Stateful tasks → Keep Sub-Agent
...
```
DO:
- Keep |S| < κ (~50 skills for flat library)
...
skills:
  ...
  - "web_lookup": "Look up information online"  # ← Duplicate!
  - "google_search": "Google something"         # ← Duplicate!
```
...
def monitor_skill_selection():
    ...
    if accuracy < 0.80:
        alert(f"⚠️ Skill selection degrading: {accuracy:.2%}")
...
def check_skill_similarity(skill_a, skill_b):
    ...
    if similarity > 0.90:
        warn(f"High confusability: {skill_a.name} ↔ {skill_b.name}")
...
class SmartRouter:
    def route(self, task):
        ...
        if complexity < threshold:
            # Simple task → Use skill (fast, cheap)
            return self.execute_skill(task)
        else:
            # Complex task → Spawn Sub-Agent (more capable)
...
print(f"OpenClaw κ = {params.κ}")  # Our capacity threshold
```
...
- Hook: "[[LLM]]s have working memory limits. Here's the math."
...
- Include: Step-by-step (decomposition → backend assignment → topology internalization)
...
### Die 3 Papers zusammen ergeben ein **vollständiges Framework für agentic [[AI]] systems**:
...
| **Framework** | 3-Layer (F/S/C) | Γ Metric + Factors | MAS ↔ SAS |
| **Optimization** | In-Context vs. Post-Training | Factor Attribution | Compilation |
| **Key Insight** | Reasoning = Interaction | Γ > 1 = Real Collaboration | Skills ≈ Agents @ 50% cost |

### Unified Decision Framework

```
┌─────────────────────────────────────────────────┐
│  TASK: Build AI Agent System for X             │
└─────────────────────────────────────────────────┘
                    │
        ┌───────────┴───────────┐
        │  Paper 2: Evaluate    │
        │  Is task decomposable? │
        │  Expected Γ > 1?       │
        └───────────┬───────────┘
                    │
          ┌─────────┴──────────┐
...
5. **Compilation Experiment** (Paper 3)
   - Pick 1 simple Sub-Agent (z.B. DEALMAKER für proposals)
   - Compile to skill
   - A/B test accuracy + cost

6. **Memory Restructure** (Paper 1)
   - Split MEMORY.md → episodic/ + semantic/ + procedural/
   - Implement memory retrieval by type

7. **Factor Attribution** (Paper 2)
   - Before adding new feature → measure Γ
...
11. **Learned Skill Selection** (Paper 3)
    - Train classifier for skill→task mapping
...
- Framework: Foundational → Self-Evolving → Collective
...
- Framework: Γ Metric + Factor Attribution
- CTA: "Try our Γ calculator"

**Part 3: "When to Use One Agent vs. Many" (from Paper 3)**
- Hook: We saved 50% on [[AI]] costs with this one trick
- Framework: MAS → SAS Compilation + Skill Scaling
...
2. **Failure Mode Analysis:** Viel über "was funktioniert", wenig über "wie/warum es crasht"
3. **Real-World Benchmarks:** Meiste Experimente auf academic tasks (GSM8K, HumanEval) — wo sind production case studies?
4. **Latency Constraints:** Niemand optimiert für real-time applications (streaming, user-facing)
5. **Human-in-the-Loop:** Alle Systeme fully autonomous — kein Framework für human oversight/correction

### Open Questions

1. **Kann man Γ > 1 predicten?** Gibt es task properties die a priori sagen "MAS wird hier gewinnen"?
2. **Optimal κ über Models?** Paper 3 hat κ für GPT-4o — gilt das für [[Claude]], Gemini, Llama?
...
4. **Beyond 2-Level Hierarchy:** Paper 3 zeigt 2-stage routing funktioniert — aber optimal depth/breadth?
5. **Compilation Losses:** Wann genau scheitert MAS→SAS? Paper 3 sagt "non-serializable" — aber konkrete failure cases?
...
**3 Papers zusammen:** Definieren die **wissenschaftliche Grundlage für agentic systems**.

→ Florian könnte der sein, der:
...
Loss = E_q [
    1/G Σ_i min(
        ρ_i * Â_i,
        clip(ρ_i, 1-ε, 1+ε) * Â_i
    ) - β * D_KL(π_θ || π_ref)
]

Â_i = (r_i - μ) / (σ + δ)  # Group-normalized advantage
```
...
where Φ_S = max_{strategy} Performance_single_agent(budget)
```

**Content Entropy:**
```
H_t = -Σ_i p(x_i | C_t) log p(x_i | C_t)

Interpretation:
- High H_t → Exploration (diverse outputs)
- Low H_t → Exploitation (converged)
- Oscillating H_t → Contextual breakdown
```

**Evolutionary Distance:**
```
D_t = Σ_i (1 - cos_sim(v_{i,t}, v_{i,t-1}))

where v_{i,t} = embedding(agent_i_output_at_t)

Interpretation:
- Low D_t → Redundant repetition
- High D_t → Semantic displacement (or breakdown)
...
Diese 3 Papers sind **must-reads** für jeden, der Production-Grade Multi-Agent Systems baut.

**TL;DR:**
- **Paper 1:** Framework (was agents können sollten)
- **Paper 2:** Metrics (wie man agents evaluated)
- **Paper 3:** Efficiency (wann single > multi)

**OpenClaw Next Steps:**
1. Implement Γ-tracking
2. Audit skill library (semantic deduplication)
3. Test compilation for DEALMAKER
4. Write Part 1 of content series

**Timeline:**
- Week 1: Implement monitoring (Γ + skill audit)
