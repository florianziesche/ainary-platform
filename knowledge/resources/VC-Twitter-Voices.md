---
type: knowledge
last_verified: 2026-02-15
status: evergreen
created: 2026-02-09
tier: KNOWLEDGE
expires: 2027-02-19
---

# [[VC]] Twitter/X Voices — Intelligence List

**Created:** 2026-02-05
**Purpose:** Daily monitoring for trends, contradictions, engagement opportunities

---

## 🔥 Tier 1: Most Active [[AI]]-Focused VCs

| Name | Fund | Handle | Followers | Activity | Notes |
|------|------|--------|-----------|----------|-------|
| **Josh Wolfe** | Lux Capital | @wolfejosh | 200K+ | ⭐⭐⭐ Daily | Deep tech, defense, [[AI]]. Threads are gold. |
| **Sarah Guo** | Conviction | @saranormous | 80K+ | ⭐⭐⭐ | [[AI]]-native, Software 3.0, No Priors podcast |
| **Elad Gil** | Angel/Investor | @eloladgil | 300K+ | ⭐⭐ | [[AI]] infra, scaling advice |
| **Garry Tan** | Y Combinator | @garrytan | 500K+ | ⭐⭐⭐ | YC CEO, startup ecosystem |
| **Sam Altman** | OpenAI | @sama | 3M+ | ⭐ Occasional | Major signal when he posts |
| **Naval Ravikant** | AngelList | @naval | 2M+ | ⭐ Rare | Philosophy + investing |
| **Balaji Srinivasan** | a16z alum | @balajis | 1M+ | ⭐⭐ | Contrarian, crypto+[[AI]] |
| **Marc Andreessen** | a16z | @pmarca | 1M+ | ⭐⭐ | Tech optimism, [[AI]] bull |
| **Paul Graham** | YC Founder | @paulg | 1.5M+ | ⭐⭐ | Essays, startup wisdom |

---

## 🏙️ Tier 2: NYC-Focused VCs

| Name | Fund | Handle | Notes |
|------|------|--------|-------|
| **Fred Wilson** | USV | @fredwilson | Daily blogger (avc.com), less Twitter |
| **Nick Chirls** | Notation | @nickchirls | Pre-seed, technical founders |
| **John Borthwick** | Betaworks | @borthwick | [[AI]] Camp, agents |
| **Ben Sun** | Primary Venture | LinkedIn mainly | NYC seed |
| **Jessica Lin** | Work-Bench | LinkedIn mainly | Enterprise tech |
| **Henri Pierre-Jacques** | Harlem Capital | @haboropij | Diverse founders |

---

## 🤖 Tier 3: [[AI]]/ML Researchers & Builders (VCs follow them)

| Name | Role | Handle | Notes |
|------|------|--------|-------|
| **Andrej Karpathy** | Ex-Tesla/OpenAI | @kaboropathy | "Vibe coding" originator |
| **Yann LeCun** | Meta [[AI]] | @ylecun | World models, AGI skeptic |
| **François Chollet** | Keras/[[Google]] | @fchollet | ARC benchmark, [[AI]] critic |
| **Jim Fan** | NVIDIA | @drjimfan | Embodied [[AI]] |
| **Swyx** | [[AI]] Engineer | @swyx | [[AI]] engineering trends |
| **Simon Willison** | Datasette | @simonw | [[LLM]] practitioner |

---

## 📰 Tier 4: [[VC]] Media & Analysts

| Name | Platform | Handle | Notes |
|------|----------|--------|-------|
| **Eric Newcomer** | Newcomer | @ericnewcomer | [[VC]] scoops |
| **Kate Clark** | The Information | @kateclark | [[VC]] news |
| **Connie Loizos** | StrictlyVC | @cookie | Deals, trends |
| **Matt Turck** | FirstMark | @mattturck | Data/[[AI]] landscape |
| **Tomasz Tunguz** | Theory | @ttunguz | Data-driven [[VC]] |

---

## 🎯 Engagement Strategy

### Daily Routine (10 min)
1. Check @wolfejosh, @saranormous, @eloladgil for new threads
2. Look for debates/contradictions
3. Engage with 1-2 quality comments

### Engagement Rules
- **Add insight, not agreement** — "Great point!" = invisible
- **Reference your experience** — Operator credibility
- **Ask smart questions** — Shows depth
- **Be early** — First 30 min matters most

### Template Comments

**When agreeing:**
> "This maps to what I'm seeing building [X]. The part that surprised me: [insight]"

**When adding nuance:**
> "Mostly agree, but there's a tension here: [counterpoint]. Curious how you think about [specific question]"

**When disagreeing:**
> "Interesting framing. I'd push back on [specific point] because [reason from experience]. But [acknowledgment of their core insight]."

---

## 🔗 Quick Links

**Twitter Lists to Follow:**
- Create private list: "[[VC]]-[[AI]]-Voices" with above handles
- List link: [create on Twitter]

**RSS Alternatives for Twitter:**
- Nitter RSS: `https://nitter.net/[handle]/rss`
- RSSHub: `https://rsshub.app/twitter/user/[handle]`

---

## 📊 Current Debates to Track

| Topic | Bull | Bear |
|-------|------|------|
| AGI Timeline | Altman, Sequoia | LeCun, Chollet |
| Job Disruption | Amodei (50%) | Huang (augmentation) |
| Vibe Coding | Karpathy | Enterprise devs |
| Software Survival | Thompson | Tunguz (debt risk) |
| Judgment Scarcity | Sarah Guo | [opportunity for you] |

---

## Related

- **↔ Pattern:** [[Content-Strategy]] — Twitter Voices als Quellen für Content-Inspirationen und Netzwerk-Engagement
- **↔ Pattern:** [[NETWORK-ACTIVATION-30DAY]] — Diese VCs als primäre Targets für LinkedIn/Twitter-Aktivierung

---

*Update weekly with new voices and debates.*